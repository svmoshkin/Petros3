...
uses
  ...
  Excel2000, Variants, ADOUtils, NumInp, ExcelXP;
       ...
       { if (Comp <= MinCompCount) then begin
        ...
        on E: Exception do begin
          ...
          if not (E is EAbort) then
            ShowMessage(stErrorInData + stAnNum + intToStr(k) + #$10 + E.Message);
          WasExcept := True;
        end;
      end;
...
var R, M: extended;
  ...
  begin
    ...
    if (DS.Lookup(FldName, Cell, 'ID') = Null) then begin
      ...
      ImpLookUpForm.Field := TStringField(DM1.GeolIds.Fields.FieldByName(FldName));
      ImpLookUpForm.DBGrid1.DataSource := DSr;
      if (StripChStr(Cell, ' ') = '') then
        ImpLookUpForm.Edit1.Text := '?'
      else
        ImpLookUpForm.Edit1.Text := Cell;
      if ImpLookUpForm.ShowModal = mrOK then
